...
class TestFace(unittest.TestCase):
    ...
    def test_get_face(self):
        r = utils.create_face('trevor', self.face['photo_id'], self.face, self.session)
        created_face = json.loads(r.content)

        r = utils.get_face('trevor', self.face['photo_id'], created_face['id'], self.session)
        self.assertEqual(r.status_code, 200)
        recv_face = json.loads(r.content)
        self.assertEqual(recv_face['bb_top_right_x'], created_face['bb_top_right_x'])
        utils.delete_face('trevor', self.face['photo_id'], created_face['id'], self.session)

    def test_get_faces(self):
        created_face1 = json.loads(utils.create_face('trevor', self.face['photo_id'], self.face, self.session).content)
        created_face2 = json.loads(utils.create_face('trevor', self.face['photo_id'], self.face, self.session).content)

        recv_faces = json.loads(utils.get_faces('trevor', self.face['photo_id'], self.session).content)
        self.assertEqual(len(recv_faces), 2)
        utils.delete_face('trevor', self.face['photo_id'], created_face1['id'], self.session)
        utils.delete_face('trevor', self.face['photo_id'], created_face2['id'], self.session)

    def test_delete_face(self):
        created_face = json.loads(utils.create_face('trevor', self.face['photo_id'], self.face, self.session).content)
        recv_faces = json.loads(utils.get_faces('trevor', self.face['photo_id'], self.session).content)
